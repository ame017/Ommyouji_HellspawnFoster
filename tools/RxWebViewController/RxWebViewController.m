...
@interface RxWebViewController ()<UIWebViewDelegate,UINavigationControllerDelegate,UINavigationBarDelegate,NJKWebViewProgressDelegate,UIScrollViewDelegate>
...
-(UIWebView*)webView{
    if (!_webView) {
        ...
        //大小在这里改
        _webView.frame = CGRectMake(_webView.frame.origin.x, _webView.frame.origin.x+self.WebViewOffSiteVertical, _webView.frame.size.width, _webView.frame.size.height-self.WebViewOffSiteVertical);
...
//        _closeButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"关闭" style:UIBarButtonItemStylePlain target:self action:@selector(closeItemClicked)];
        _closeButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"nav_close_18x18_"] style:UIBarButtonItemStylePlain target:self action:@selector(closeItemClicked)];
    }
